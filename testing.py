...
class TestIkeaScraper(TestCase):   
    ...
    def test_get_list_of_links(self):
        self.obj_scraper.launch_homepage()
        result_links = self.obj_scraper.get_list_of_links(1)        
        self.assertIsInstance(result_links, list) # list returned is in format 'list'
        # self.assertEqual(len(result_links), 46) # number of items returned are 46 (no longer applicable)
    ...
    def test_make_make_product_filess(self):
        product_links = [
        ...
        dictionary_list = self.obj_scraper.make_product_files('./raw_data/', product_links)
